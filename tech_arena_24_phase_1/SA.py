...
class SimulatedAnnealing:
    def __init__(self, initial_solution, initial_temp, min_temp, alpha, max_iter):
        ...
        self.id_gen = ThreadSafeIDGenerator(start=1)
    ...
    def generate_neighbor(self, current_solution, slot_manager:SlotAvailabilityManager, servers, datacenters):
        ...
        if max_available_slots >= slots_size:
            max_purchase_count = max_available_slots // slots_size // 2 # 最多购买一半的插槽
...
if __name__ == '__main__':
    ...
    # 初始化模拟退火算法
    initial_solution = pd.DataFrame(columns=['time_step', 'datacenter_id', 'server_generation', 'server_id', 'action'])
    S = DiffSolution(seed=seed)
    sa = SimulatedAnnealing(initial_solution, initial_temp=10000, min_temp=1, alpha=0.99, max_iter=200)

    # 运行算法
    best_solution, best_score = sa.run(S, servers, datacenters)

    save_solution(best_solution, f'./output/{seed}_{best_score}.json')
