...
class SimulatedAnnealing:
    ...
    def _adjust_life_expectancy(self, new_solution, selected_action, servers, adjustment_direction):
        ...
        if adjustment_direction == "decrease":
            ...
            # 初始化新裁撤时间和寿命
            new_real_dismiss_time = None
            max_search_steps = 100  # 限制最大搜索步数，防止搜索时间过长
            new_dismiss_life = current_dismiss_life

            # 从当前时间点往后查找，找到满足插槽需求的最早时间点
            for future_time_step in range(buy_time_step + 1, buy_time_step + max_search_steps):
             # 获取未来时间步的插槽可用数量
                available_slots = self.get_available_slots_at_time(selected_action['datacenter_id'], future_time_step)
        
            # 如果当前时间步的可用插槽数量足够满足需求，则记录新的裁撤时间和寿命
                if available_slots >= total_slots_needed:
                    new_real_dismiss_time = future_time_step
                    new_dismiss_life = future_time_step - buy_time_step
                    break  # 找到满足条件的时间点后退出循环

         # 判断是否需要更新裁撤寿命（如果新时间点比当前时间点更优）
        if new_real_dismiss_time and new_real_dismiss_time > real_dismiss_time:
             # 更新新的寿命期望和退役时间
            new_solution.at[selected_action.name, 'dismiss_life_expectancy'] = new_dismiss_life
        ...
        # 打印标准信息
            ...
            self._print(f"没有找到符合插槽需求的时间点，未进行裁撤寿命的调整。")
