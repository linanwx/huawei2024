...
def get_capacity_by_server_generation_latency_sensitivity(fleet):
    Z = fleet.groupby(by=['server_generation', 'latency_sensitivity',], observed=True)['capacity'].sum().unstack()
...
def get_cost(fleet_info:FleetInfo):
    ...
    return cost.sum() + fleet_info.purchase_cost
...
class DiffSolution:
    ...
    def SA_evaluation_function(self, diff_input:DiffInput, time_steps=TIME_STEPS):
        ...
        # fleet = fleet.set_index(['server_generation', 'latency_sensitivity'])
